...
from sage.all import Matrix, vector, identity_matrix, GF, Integers, matrix, ZZ, ones_matrix
...
def EigenspaceGuess(n, m, ke23, MO, kappa, verbose=True):
    ...
    for j in range(n):
        ...
        if verbose:
            print("j=", j, "len(li)=", len(li), "tot=", sum(v.nrows() for v in li), "max=", ma)
        if ma == 1:
            break
        ...
        for e in range(2):
            if kappa > 0 and j == 0 and e == 1:
                continue  # only do this for constant Hamming weight
            Me = M[:, :]
