...
from sage.all import Matrix, ZZ, identity_matrix, log, RealField, RDF, GF, inverse_mod, mod, round
...
def Step1_plus_gen(n, v, m=0, BKZ=False):
    ...
    x0, a, X, b = building.genParams(n, m, nx0, v)

    M = building.orthoLatticeMod(b, n, x0)
    ...
    print("  Kernel LLL: %.1f" % time.time() - tk),

    for i in range(2, m / n):
        MO[:, i * n : (i + 1) * n] = -(M[i * n : (i + 1) * n, : 2 * n] * MO[:, : 2 * n].T).T
    # print "Total kernel computation",time.time() - tk
    tt1 = time.time() - t
    tt1O = time.time() - tk
    ...
    del M, mv
    if BKZ:
        return MO, x0, a, X, b, int(m), tt1, tt10, tt1O
    else:
        return MO, x0, a, X, b, int(m), tt1
